...
export default async function RequestsPage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string; status?: string }> }) {
  ...
  return <div><h1 className="text-3xl font-black">Demandes</h1><p className="mt-2 text-slate-500">Suivez les réservations et prestations de bout en bout.</p><form className="mt-6 grid gap-3 rounded-2xl border bg-white p-4 sm:grid-cols-[1fr_220px_auto]"><input aria-label="Rechercher des demandes" className={inputClass} name="q" defaultValue={query} placeholder="Client, e-mail ou objet…" /><select aria-label="Filtrer par statut" className={inputClass} name="status" defaultValue={parsedStatus.success ? parsedStatus.data : ""}><option value="">Tous les statuts</option><option value="SUBMITTED">Reçues</option><option value="CONFIRMED">Confirmées</option><option value="IN_PROGRESS">En cours</option><option value="COMPLETED">Terminées</option><option value="CANCELLED">Annulées</option></select><Button type="submit">Filtrer</Button></form><Card className="mt-6"><CardHeader><CardTitle>{total} demande(s)</CardTitle></CardHeader><CardContent><div className="grid gap-3">{rows.map((request) => <div key={request.id} className="grid gap-3 rounded-xl border p-4 sm:grid-cols-[1fr_auto_auto] sm:items-center"><div><Link href={`/dashboard/requests/${request.id}`} className="font-semibold text-brand-950 hover:text-brand-600">{request.subject}</Link><p className="mt-1 text-xs text-slate-500">{request.customerName} · {request.email} · {request.kind}</p></div><p className="font-bold">{money(request.total)}</p>{canManage ? <form action={updateRequestStatus} className="flex gap-2"><input type="hidden" name="requestId" value={request.id} /><select name="status" defaultValue={request.status} className={inputClass}><option value="SUBMITTED">Reçue</option><option value="CONFIRMED">Confirmée</option><option value="IN_PROGRESS">En cours</option><option value="COMPLETED">Terminée</option><option value="CANCELLED">Annulée</option></select><Button size="sm">OK</Button></form> : <Badge>{request.status}</Badge>}</div>)}{!rows.length && <p className="py-10 text-center text-slate-500">Aucun résultat.</p>}</div><Pagination page={page} pages={totalPages(total)} pathname="/dashboard/requests" params={{ q: query, status: parsedStatus.success ? parsedStatus.data : undefined }} /></CardContent></Card></div>;
}
