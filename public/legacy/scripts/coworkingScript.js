...
export default function pagination(){
    ...
    const recap = document.getElementById("duree_total");
    ...
    nextButton2.addEventListener("click", event => {
        ...
        const nbrPersonne = parseInt(document.getElementById('nbrPers').value);   
    
        recap.innerText = nbrPersonne + ' Personne(s) pour une durée d\'un(e) ' + temps;
        // total.innerText = 'Total à payer : ' + parseFloat(nbrJour.value * 9000).toFixed(2) + ' Da';
        if(temps==="Journee"){
            if(nbrPersonne<=2){
                total.innerText = 'Total à payer : ' + parseFloat(nbrPersonne * 1000).toFixed(2) + ' Da';
            }else{
                total.innerText = 'Total à payer : ' + parseFloat(nbrPersonne * 667).toFixed(2) + ' Da';
            }
        }
        else if(temps==="Semaine"){
            if(nbrPersonne<=2){
                total.innerText = 'Total à payer : ' + parseFloat(nbrPersonne * 4000).toFixed(2) + ' Da';
            }else{
                total.innerText = 'Total à payer : ' + parseFloat(nbrPersonne * 2667).toFixed(2) + ' Da';
            }
        }
        if(temps==="Mois"){
            if(nbrPersonne<=2){
                total.innerText = 'Total à payer : ' + parseFloat(nbrPersonne * 15000).toFixed(2) + ' Da';
            }else{
                total.innerText = 'Total à payer : ' + parseFloat(nbrPersonne * 1000).toFixed(2) + ' Da';
            }
        }
