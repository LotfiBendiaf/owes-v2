...
export function Badge({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return <span className={cn("inline-flex bg-rose-50 px-2.5 py-1 font-mono text-[10px] font-semibold uppercase tracking-wider text-rose-900", status?.tone, className)}>{status?.label ?? children}</span>;
}
