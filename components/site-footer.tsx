    ...
    <footer className="border-t border-brand-950/15 bg-[#f4f0e7] text-brand-950">
      <div className="mx-auto max-w-7xl px-6 py-14 sm:py-20">
        ...
        <div className="grid gap-10 py-12 md:grid-cols-2 lg:grid-cols-[1.15fr_.65fr_1fr]">
          ...
          <div><p className="font-mono text-[10px] uppercase tracking-[.2em] text-slate-500">Index</p><nav className="mt-5 grid gap-3">{links.map((link, index) => <Link key={link.href} href={link.href} className="group flex items-center gap-3 text-sm font-bold"><span className="font-mono text-[10px] text-slate-600">0{index + 1}</span><span className="transition group-hover:text-brand-500">{link.label}</span></Link>)}</nav></div>
